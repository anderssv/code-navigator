...
2. **New `--exclude-packages=<regex>` param**, giving a manual backstop for hub packages the composition-root heuristic doesn't catch (or that aren't composition roots at all, like test-only wiring). Chosen as a *distinct* param name rather than reusing the shared `EXCLUDE` param, on two grounds: (a) converge's exclusion matches **package names** in intersect mode (`dep.sourcePackage`/`targetPackage`), not just class names — so `--exclude-packages` names what it actually does more honestly; and (b) `cnav-config.json`'s `defaults` section is flat, keyed purely by param name with no per-task allowlist, so a shared `defaults.exclude` would silently cross-apply to `cnavDead`/`cnavTestCoupling` too — which filter *different* things (dead-code items / test classes to skip), so one project-wide exclude list would be surprising even though the match mechanics are the same. (Note: an earlier version of this write-up claimed `EXCLUDE` had *inconsistent match semantics* between `cnavDead` and `cnavTestCoupling` — that was wrong. Both use substring `containsMatchIn` on the fully-qualified class name; `cnavDead`'s `item.className.matches(exclude)` calls the project's `ClassName.matches` helper, which is `containsMatchIn`, not the stdlib full-string `Regex.matches`. So `EXCLUDE` is already semantically reconciled across its consumers; the distinct-name decision rests only on the two grounds above, not on any semantic mismatch.) `--exclude-packages` filters the dependency graph *before* cycle/ring detection runs (not just the final edge list) — a hub package's edges need to be gone before SCC computation for the fix to actually work, not just hidden from display afterward. Applies uniformly in both modes: intersect mode drops any `PackageDependency`/resolved coupling pair touching an excluded package; risk mode drops any matching class from the ranking.
