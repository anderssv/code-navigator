...
### ~~`cnavRings` evidence/hint coverage gaps — composition roots and project-class evidence~~ — DONE (v0.1.115-SNAPSHOT)
...
**Follow-up, resolved**: re-read `AppDependencies`' actual source before designing that third hint branch — it declares a *public* `val objectMapper: ObjectMapper` directly on the interface, which is a genuine, correctly-classified `FRAMEWORK_SIGNATURE` finding on its own merits (nothing to do with the composition-root question). No third hint branch needed; the case that prompted it wasn't actually a gap once checked against the whole file, not just the specific evidence line the tool printed — a reminder that a blind subagent's diagnosis of *why* a violation looks wrong is not automatically correct just because the violation itself is worth double-checking.

Went back to the two deliberately-unfixed ambiguous candidates (`JsonNode`, `GenericObjectPoolConfig`) and read their actual call sites in bass-ra-backend: both are used purely as inert data/config value types with no I/O in this codebase, confirming the earlier caution was warranted but the specific instances are safe. Rather than a package-wide `valuePackages` entry (rejected above, for good reason — both containing packages have real adapter-relevant siblings: `ObjectMapper`, `GenericObjectPool`/`PooledObjectFactory`), added a new exact-match `EXACT_VALUE_TYPES` set (folding in the pre-existing `java.io.Serializable` narrow exclusion) — same narrowing principle, generalized from one entry to three. New contrast-pair tests confirm the siblings (`ObjectMapper`, `GenericObjectPool`) remain real signals.

While investigating `AppDependencies`, the second fresh subagent found one more real gap, independent of the above: a *private* factory method inside its companion object (`private fun createOtpHttpClient(): HttpClient`) was contributing a `FRAMEWORK_SIGNATURE` finding to the enclosing interface, because `SignatureTypeScanner` visited every field/method regardless of visibility and Kotlin companion-object members compile onto the outer top-level class. A private member's type is never visible to a caller of the class — same weight as a type touched only inside a method body, not a real signature exposure. Fixed by skipping `ACC_PRIVATE` fields/methods in `SignatureTypeScanner`. Didn't change bass-ra-backend's violation count (the unrelated public `objectMapper` property already made that specific classification correct), but is a general correctness fix, verified via a new test-project fixture (`ClassWithPrivateHelper`) and full suite pass with no regressions — the pre-existing `JdbcUserRepository` signature test still passes, since `Connection` there is captured via the public constructor, not the private backing field.

### ~~`cnavRings` field-tested against Spring PetClinic (Java/Maven, Spring Data JPA) — composition-root exclusion, project-namespace collision, Serializable, and Spring Data proxy ports~~ — DONE (v0.1.115-SNAPSHOT)
~~**ACTIVE**~~ **DONE (v0.1.115-SNAPSHOT)** | **Value: high** | **Effort: medium** | Source: field-test(spring-petclinic)

First Java/Maven field test of the reworked `cnavRings` (all prior field tests — greitt, kotlin-htmx, bass-ra-backend — were Kotlin/Gradle). Surfaced three real bugs plus one genuine, principled limitation worth a design decision, in sequence as each was fixed and the next became visible:

1. **`CompositionRootDetector` didn't exclude already-classified adapters from composition-root candidacy.** A Spring `@Controller` is invoked reflectively by the framework's annotation-based routing — nothing in the compiled bytecode ever calls it — so it satisfies "unreferenced + reaches an adapter" exactly like a true composition root (`main()`/`Application.kt`) does. Every controller in PetClinic was misclassified as an assembler and excluded from the graph entirely before ring detection ran, producing `adapters: []` / `compositionRoots: [OwnerController, PetController, ...]`. Fixed with one added filter (`it !in graph.ioClasses`) in `CompositionRootDetector.detect` — a composition root is a plain assembler, never itself framework-driven; the two categories are mutually exclusive by definition.

2. **`frameworkFindingFor`'s signature-type check didn't filter out project-internal classes before matching a framework prefix.** `signatureTypes` deliberately carries every type named in a signature position regardless of whether it's project-internal or external (that's what makes it useful elsewhere) — but a framework signal must be a real *external* type. Harmless for almost every real project, but Spring PetClinic's own root package is literally `org.springframework.samples.petclinic` — nested under the exact bare prefix (`"org.springframework"`, no trailing dot) used to detect real Spring usage — so any class referencing any other class in the project self-matched the framework prefix. ~20 of 30 classes were misclassified as adapters purely from touching each other. Fixed by filtering `signatureTypes[cls]` (and, defensively, `externalDeps`) to `!in projectClasses` before the framework-prefix check, in both `detectFrameworkAdapters` and `findingFor`.

3. **`java.io.Serializable`** (a pure JDK marker interface — declares serialization eligibility, performs no I/O itself) was triggering `SINK_WITH_EXTERNAL_CALLS`, since it wasn't excluded by any existing non-adapter-signal list. `BaseEntity implements Serializable` cascaded into the entire JPA entity hierarchy (`Person`, `NamedEntity`, `Pet`, `Vet`, ...) being reported as domain→service violations. Fixed with a narrow, *exact-match* exclusion (`JDK_MARKER_INTERFACES = setOf("java.io.Serializable")`), deliberately not a `"java.io."` package-prefix exclusion — `java.io.File`/`java.io.InputStream` are real filesystem I/O and must keep counting as adapter signals, unlike the earlier `java.nio.charset`-vs-`java.nio.file` and `javax.xml.datatype`-vs-broad-`javax.` precedents.

4. **Genuine limitation, not a bug — implemented as a new feature.** Even after all three fixes, PetClinic still reported "1 ring, no boundaries": Spring Data JPA generates repository implementations via a runtime dynamic proxy (`interface OwnerRepository extends JpaRepository<Owner, Integer>`, no `OwnerRepositoryImpl` class ever compiled). Static bytecode analysis structurally cannot see that generated implementation, so the repository *interface itself* was the only visible framework-touching thing (correctly, since it does extend a real Spring type) — collapsing "port" and "its own adapter" into one class instead of a real port/adapter pair, and hiding the inversion boundary entirely. New `ProxyPortDetector` recognizes interfaces with zero compiled implementors that extend a known proxy-generating supertype (Spring Data JPA/Mongo/Elasticsearch/Redis repositories, Quarkus Panache repositories) and injects a synthetic implementor (`<Interface>$GeneratedProxy`) standing in for the proxy cnav can never see directly — always labeled `[synthetic — framework-generated implementation, not a compiled class]` wherever printed, so it's never mistaken for a real class. New `AdapterReason.FRAMEWORK_GENERATED_PROXY`.

**Verified end-to-end on spring-petclinic**: went from `adapters: []`, `compositionRoots: [6 controllers]`, "1 ring, no boundaries" (fix #1 alone) → same "1 ring" but now with 6 spurious domain→service violations from the `Serializable` cascade (fix #2 alone, before #3) → those violations gone (fix #3) → **2 rings, zero violations** ("every dependency points inward") once `ProxyPortDetector` correctly recognized the three Spring Data repositories as ports (fix #4). A clean, correct, meaningfully hexagonal read of the official Spring Boot reference application.

**Noted but deliberately not fixed at the time**: `CrashController`/`WelcomeController` (trivial controllers with no framework-typed parameters/return values — just `@GetMapping` returning a view-name `String`) stayed in the innermost ring, since `SignatureTypeScanner` deliberately never visits annotations, only real declared supertypes/field/parameter/return types. Fixed later — see the RealWorld/Conduit field test below, which hit the same gap with real consequences (composition-root misclassification, not just a missed adapter tag) and added a dedicated annotation-based signal.
...
### `cnavRings` field-tested against a Java/Maven RealWorld/Conduit implementation — annotation-based framework entry-point detection
**DONE (v0.1.115-SNAPSHOT)** | **Value: high** | **Effort: medium** | Source: field-test(realworld-springboot)

Second Java/Maven field test (after Spring PetClinic). Confirmed the "annotation-only, no framework-typed signature" scope boundary noted-but-not-fixed above actually recurs with real consequences on a different codebase, this time affecting six genuine `@RestController` classes (`ArticleController`, `ArticleFavoriteController`, `CommentController`, `ProfileController`, `TagController`, `UserController`, plus `AuthController`) — not just a missed adapter tag, but a full misclassification as a composition root: each controller's fields/parameters/return types were entirely project DTOs/services, invisible to `SignatureTypeScanner`, and since the framework dispatches to a controller via reflection, nothing in the project's own bytecode calls it either — satisfying "unreferenced + reaches an adapter" exactly like a true assembler.

Fixed with a new, independent signal, checked before every other adapter check: a curated, deliberately narrow set of framework annotations that exist *specifically* to mark an HTTP/RPC entry point — `@RestController`, `@Controller` (Spring), `@Path` (JAX-RS/Quarkus) — now classify a class as an adapter directly via new `AdapterReason.FRAMEWORK_ENTRY_POINT_ANNOTATION`, regardless of what its signature looks like. Deliberately excludes generic stereotypes (`@Component`, `@Service`, `@Bean`, plain `@Configuration`) — those mark the overwhelming majority of ordinary DI beans, which are domain/service code, not adapters; a blanket annotation-based rule broad enough to catch those would misclassify far more than it fixes.

Wiring: `AnnotationExtractor.scanAll(classDirectories).classAnnotations` (already existed, previously unused by the rings pipeline) is threaded through `RingsOrchestrator` → `RingGraphBuilder.build` → both `AdapterDetector.detectFrameworkAdapters` and `AdapterDetector.detect` as a new `classAnnotations: Map<ClassName, Set<AnnotationName>>` parameter — no new bytecode scanner needed.

**Verified against a real counter-example on this same codebase**: `AuthorizationConfig`, a `@Component` (not `@Configuration`) holding real business logic, invoked only via Spring Security SpEL string expressions (`@PreAuthorize("@authorizationConfig.isArticleAuthor(#slug)")`) — never a direct Java call. It correctly stays a composition-root candidate rather than getting the new annotation-based adapter tag, since `@Component` is deliberately excluded from `ENTRY_POINT_ANNOTATIONS`. This is a genuinely harder, separate, out-of-scope case (reflective dispatch via a SpEL string, not an annotation cnav could recognize) — a fresh subagent independently flagged it during blind validation and confirmed the reasoning, without being told about it in advance.

**Verified end-to-end**: went from 6 controllers misclassified as composition roots (only 2 genuine assemblers visible: `AuthorizationConfig`, `CustomHandlersConfig`) to all 6 correctly reclassified as ring-1 adapters with the new evidence tag, composition roots narrowed to the 2 genuine candidates, zero violations, a clean 2-ring hexagonal read — unchanged from before the fix (the misclassification hid controllers from ring assignment entirely rather than corrupting the violation count, since none of them happened to have an outward-pointing edge).

**Tests**: `AdapterDetectorTest` — 2 new (a Spring `@RestController` with an entirely project-typed signature is a framework entry point; `@Component` alone is not). No `RingGraphBuilderTest`/`RingsOrchestratorTest` changes needed — the new parameter defaults to `emptyMap()` and threads through existing call sites unchanged.

**Environment note, unrelated to cnav**: this project's Lombok annotation processing is incompatible with very new JDKs (tested broken on JDK 26 — `cannot find symbol: method builder()` etc. across every Lombok-annotated class); compiling it for testing requires an explicit `JAVA_HOME` pointed at JDK 21 (its own declared `<java.version>`), not whatever a machine's newest installed JDK happens to be.


### ~~`cnavRings` field-tested against Quarkus microservices (`quarkus-super-heroes`) — JAX-RS/MicroProfile Rest Client proxy conflict, plus a round of package-list gaps~~ — DONE (v0.1.115-SNAPSHOT)
**DONE (v0.1.115-SNAPSHOT)** | **Value: high** | **Effort: medium** | Source: field-test(quarkus-super-heroes)

Field-tested the new entry-point-annotation signal (added for the RealWorld/Conduit test above) against a real Quarkus codebase using JAX-RS, gRPC, MongoDB Panache, Kafka/Avro, and MicroProfile Rest Client together — a much wider framework surface than any single prior field test.

**Real conflict found and fixed**: JAX-RS `@Path` marks a *server-side* resource as a driving adapter (`FRAMEWORK_ENTRY_POINT_ANNOTATION`) — but a MicroProfile Rest Client interface (`@RegisterRestClient`, implemented by a runtime-generated proxy, structurally the exact same pattern as a Spring Data repository) frequently carries the identical `@Path` annotation to describe an *outbound* call. Without a fix, `HeroRestClient`/`NarrationClient` (both genuine client-side ports) were misclassified as the adapter themselves. Fixed by extending `ProxyPortDetector` — previously supertype-only (Spring Data/Panache) — to also recognize `@RegisterRestClient`-annotated interfaces with zero compiled implementors. No change needed in `AdapterDetector` itself: `RingGraphBuilder` already excludes anything `ProxyPortDetector` claims from the adapter-finding set, so the existing dedup resolved the conflict for free once the detector recognized the annotation. Took one microservice from a flat "1 ring, no boundaries" (no port anywhere was visible) to a correct 2-ring read with a real inversion boundary.

**A round of package-list gaps, found and fixed iteratively as each fix surfaced the next "evidence hop"** (the same class's classification would survive one fix but pick a different, equally-wrong piece of evidence from further down its external-dependency list) — a instructive pattern in itself: fixing evidence-selection one type at a time can chase a moving target rather than resolving the root classification:
1. `io.quarkus.logging.Log` / `org.jboss.logging.` matched the bare `"io.quarkus"` prefix instead of being excluded like `org.slf4j` — added to `LOGGING_PACKAGES`. This one mattered beyond cosmetics: it was silently outranking the *real* reason a class was or wasn't an adapter, since a project-internal port-interface call (invisible to `externalDeps`) left logging as the only visible signal. Fixing it correctly reclassified `HeroClient` and `FightService` from "adapter" to "service tier calling a port" — the actually-correct hexagonal read.
2. `com.google.protobuf.` (protoc-generated `*OrBuilder` interfaces, the per-`.proto` `GeneratedFile` descriptor holder) and `org.apache.avro.` (Avro's `SpecificRecordBase`/`SpecificRecordBuilderBase`, the per-class `BinaryMessageEncoder`/`Decoder` static fields every generated record carries) are both pure wire-format value/codec metadata — the real transport is a separate, uncovered package (`io.grpc.`/`io.quarkus.grpc.` for gRPC; the surrounding Kafka connector for Avro). Added to `VALUE_LIBRARY_PACKAGES`. This one had real consequences beyond a single class: the generated gRPC/protobuf types cascading into false domain→service violations disappeared entirely once fixed (4 domain→service violations → 0).
3. `jakarta.ws.rs.WebApplicationException`/`javax.ws.rs.WebApplicationException` (inspected for its status code, not used for I/O), `jakarta.ws.rs.core.Response`/`javax.ws.rs.core.Response` (a value/builder object — closer to `JsonNode` than to `ObjectMapper`; the real HTTP write happens later, inside the JAX-RS runtime), and `java.nio.ByteBuffer` (a pure in-memory byte container — real I/O happens via a separate `java.nio.channels.*` channel) added as exact-match value types, resolving a `Predicate<Throwable>` class (`Is404Exception`) that had no I/O of its own but kept picking a new piece of evidence each time the previous one was excluded.
4. Conversely, `io.smallrye.reactive.messaging.` (Quarkus/SmallRye's Kafka/AMQP `MutinyEmitter`) was *missing* from `FRAMEWORK_PACKAGES` entirely — a service class that genuinely calls `emitter.sendAndForget(...)` to publish to Kafka had no signal to be classified by at all. Verified against the actual call site before adding it, not just the field type.

**Deliberately left one hop unfixed**: after fix #2 and #3, the same Avro-generated `schema.Fight` class's `throws java.io.IOException` declaration (on its generated `toByteBuffer()`/`fromByteBuffer()` methods) still triggers classification. Unlike the previous fixes, a checked-exception declaration is a much weaker "no I/O happens here" signal — plenty of real I/O methods also declare `throws IOException` — so this is documented as a known residual false positive rather than excluded, consistent with the project's standing discipline of not excluding a package/type once the "definitely no I/O" case gets genuinely debatable (same reasoning that earlier stopped a broad `JsonNode`/`GenericObjectPoolConfig` exclusion at exact-match instead of package-wide).

**Verified with two rounds of fresh, zero-context subagent validation** per the established pattern (one after the proxy-detector fix alone, one after the full round of package-list fixes) — both independently re-derived several of the fixes above from source before being told about them, and the second round's own investigation directly led to the `Log`/`Response`/Avro findings (the subagent flagged `Log` as a misleading evidence string; investigating *why* led to the fix, not just a config workaround).

**Tests**: `ProxyPortDetectorTest` — 2 new (`@RegisterRestClient` interface with no implementor is a proxy port; with a real implementor, is not). `RingGraphBuilderTest` — 1 new (the `@Path`/`@RegisterRestClient` conflict resolves to a port+synthetic-proxy pair, not an adapter). `AdapterDetectorTest` — 7 new (Quarkus logging exclusion; `WebApplicationException`; `Response`; `ByteBuffer`; Avro generated-schema exclusion; SmallRye reactive messaging as a real signal — each with a contrast case where the sibling type in the same package stays a real signal where applicable).


### ~~`cnavMoveFile`/`cnavMoveClass --from-file`/`cnavMovePackage`/`cnavExecutePlan`: under-reached on former same-package callers, the inverse of the 67-file over-reach above~~ — DONE (v0.1.116-SNAPSHOT)
**DONE** | **Value: high** | **Effort: medium** | Source: field-test(greitt)

Reported directly against the 67-file over-reach item above: same root cause (the rewriter's *caller set* is wrong), opposite direction — this time too few files were touched, not too many. `cnavMoveFile` reported "applied, 2 files" and compilation broke: a same-package caller of a moved top-level function had no import to rewrite (none was needed while both files shared a package), and none was added.

**Root cause, once traced through**: three separate rewrite paths in `MoveClassRewriter` (`moveClass`'s sibling handling, `moveKtFacade` — the path a pure top-level file with no declared classes actually takes — and `moveMultiClassFile`) each computed the *moved file's* real top-level function/property names correctly (`extractTopLevelNames`), but only ever fed that into logic that rewrites an **existing** import in another file. None of them fed those names into `importMovedTypeInFormerSamePackage` — the one function whose actual job is "does a former same-package caller need a *brand-new* import added" — which only ever received the moved class's simple name (or, for a pure top-level file, the synthetic `FileNameKt` facade name, a token that never literally appears in a caller's source; Kotlin callers write the bare function name, not the file facade). A same-package caller referencing only the real function name was therefore invisible to every check that could have added it an import.

The batch/`CompositeRecipe` path (`runBatchedMoves`, used by `cnavMovePackage`/`cnavExecutePlan`) had the identical gap independently, plus a second one: its own logic for attributing a changed file back to the specific move that caused it only recognized files that already contained the *old* import string — a file gaining a brand-new import (never had the old one) fell through attribution entirely, so even after adding the missing-import logic there, the change would silently not show up in that move's own preview/dry-run result (though it would still be written to disk on a real, non-preview run, since that step iterates every changed file unconditionally).

**Fix**: `importMovedTypeInFormerSamePackage` (List-based, for `moveClass`/`moveKtFacade`/`moveMultiClassFile`) now takes the moved class's simple name *plus* `extractTopLevelNames` on the moved file's original content, not just the class name alone. New Map-keyed twin `importMovedTypeInFormerSamePackageMap` added for `runBatchedMoves`'s shared-changes map, called with the same name set per move — and its return value (the set of paths it touched) is threaded into the batch's existing attribution loop as `extraRelevantPaths`, so a brand-new same-package import is correctly attributed to the move that caused it, in preview mode as well as real runs.

**Tests**: `MoveFileRewriterTest` — 1 new (pure top-level file via the Kt-facade path). `MoveClassRewriterTest` — 1 new (multi-class file with a co-located top-level function). `MoveClassRewriterBatchTest` — 1 new (a moved class itself, via the batch path). New test-project fixtures: `SvgIcons.kt`/`CalendarComponents.kt` (top-level function, same-package caller), a top-level function added to the existing `Events.kt` multi-class fixture plus new `EventLogger.kt` caller, and new `TaxCalculator.kt`/`TaxCalculatorUser.kt` (plain class, same-package caller, batch path).

### ~~`cnav-config.json`'s `rings.valuePackages` only suppressed one of the three adapter-classification rules~~ — DONE (v0.1.116-SNAPSHOT)
**DONE** | **Value: medium** | **Effort: low** | Source: field-test(greitt)

Reported directly against real use of the config option: declaring `jakarta.validation.` a value package (to stop bean-validation annotations from triggering false adapter classifications) fixed classes that merely *called* into that package (`SINK_WITH_EXTERNAL_CALLS`), but not `ValidationService` (names `jakarta.validation.Validator` in its own signature — `FRAMEWORK_SIGNATURE`) or `HtmlConstraints` (references a `@NotBlank` annotation type internally — `FRAMEWORK_TYPE`), both of which had to be separately worked around with `notAdapters`.

**Root cause**: `extraValuePackages` (the config-provided extension to `VALUE_LIBRARY_PACKAGES`) was threaded into `isLibraryType` (backing `SINK_WITH_EXTERNAL_CALLS` only) but never into `isFrameworkType` (backing `FRAMEWORK_SIGNATURE`/`FRAMEWORK_TYPE`), unlike the *built-in* `VALUE_LIBRARY_PACKAGES` list, which `isFrameworkType` already carved out ahead of a broad framework-prefix match (that's the exact mechanism that lets `javax.xml.datatype` win over bare `javax.`). A project-configured value package silently didn't get the same treatment as a built-in one.

**Fix**: `isFrameworkType` now also accepts `extraValuePackages` and checks it in the same carve-out as `VALUE_LIBRARY_PACKAGES`/`LOGGING_PACKAGES`, threaded through `frameworkFindingFor`/`detectFrameworkAdapters`/`RingGraphBuilder.build`. A package declared pure via `cnav-config.json` is now exempt from all three adapter rules uniformly, matching the built-in list's behavior. Verified end-to-end against greitt with the `notAdapters` workarounds removed: both classes now correctly land in ring 0 from `valuePackages` alone.

**Tests**: `AdapterDetectorTest` — 2 new (`extraValuePackages` suppressing `FRAMEWORK_SIGNATURE`; suppressing `FRAMEWORK_TYPE`).


### ~~`cnavTestCoupling --subject=adapters`: use-case orchestration hiding in a driving adapter~~ — DONE (v0.1.116-SNAPSHOT)
**DONE** | **Value: high** | **Effort: high** | Source: field-test(greitt, independent LLM code review)

Raised directly against a real gap: an independent reviewer (an LLM agent with no access to cnav's output) read greitt's source cold and found real use-case orchestration hiding in `ParticipantRoutes.kt`/`AdminRoutes.kt` (validate → mutate → persist → send-email sequences, including a retry loop, driven straight from a Ktor route handler) — invisible to every existing cnav check, since the route handler calling a port directly is a structurally healthy edge by every direction-based measure. The reviewer's own summary: *"the hexagon is violated by what's absent from the inside, and absence has no edge."*

**The insight the fix generalizes**: `cnavTestCoupling`'s existing machinery (find a caller directly calling a port method instead of through the domain service that owns it, per Testing Through The Domain) is *exactly* the right check, aimed one layer over — from test classes to driving adapters. Its own existing fix text ("replace direct port/adapter calls with the equivalent domain service operation") was already the right advice; it just needed a second class of subject.

**Design decisions, in the order the field feedback ranked them**:
1. **Only mutating (write) port methods count** — read-side pass-through from a handler (`repository.get(id)` called straight from a route) is idiomatic and harmless; flagging it would bury the real signal. New `PortMethodClassifier`: a name-prefix heuristic (`add`/`update`/`delete`/`save`/`create`/`insert`/`remove`/`finalize`/`migrate`/`persist`/`publish`/`send`/`register`/`revoke`/`archive`/`schedule`/`cancel`/`set`), with `--write-methods`/`--read-methods` (`Iface.method`, comma-separated) overrides for names the heuristic gets wrong in either direction.
2. **The subject is "a driving adapter with no application/service tier on the call stack"** — reuses `cnavRings`' own adapter/service-tier classification (`RingsOrchestrator` → `ioClasses - serviceTier - $GeneratedProxy` synthetic implementors), further narrowed in `TestCouplingBuilder.analyze` to exclude implementors of the configured `--ports` regex itself (a port's own implementor calling its own interface isn't the shape this check is for). This framing also correctly *excludes* a service calling a port write — the entire point of having a service tier.
3. ~~Report the full call chain through compiler-generated coroutine/lambda wrappers~~ — already handled for free: `TestCouplingBuilder`'s existing `outerClassName` unwrapping (used for test classes) collapses `AdminRoutesKt$configureAdminRoutes$3$12.invokeSuspend` back to `AdminRoutesKt` the same way for adapter callers, with no separate work needed.
4. **Gateable** — added `--fail-on-violation`/`--max-violations` (reusing the existing `cnavRings`/`cnavCycles` parameters) so a team can pin today's count and ratchet down as services absorb the logic, the same CI pattern `cnavRings` already established.

**Implementation**: new `CouplingSubjectKind` (TEST/ADAPTER) on `TestCouplingViolation`; `TestCouplingConfig` gains `subjects`, `candidateAdapterClasses` (precomputed by the orchestrator, not the builder), `writeMethods`/`readMethods`. `TestCouplingBuilder.analyze`'s single edge-walk now branches per subject kind — test-subject behavior is completely unchanged (verified: no regressions in any pre-existing test). New `--subject=tests|adapters|both` parameter (default `tests`, preserving prior behavior exactly). `TestCouplingOrchestrator.run` gained a `projectDir` parameter and only pays for computing rings when the adapter subject is actually requested. Formatter/guidance text updated to describe both subjects and cross-reference "run `cnavRings` first if the split looks wrong."

**Verified end-to-end on greitt**: `cnavTestCoupling --ports=".*Repository" --subject=adapters --detail=true` reproduced the independent reviewer's finding exactly — `AdminRoutesKt` × 6 `updatePoll` + 1 `deletePoll` + 1 `finalizePoll`, `ParticipantRoutesKt` × 2 `updatePoll` (10 total, matching the reviewer's manual `cnavFindUsages`-based count method-for-method) — while `DateToggleService`/`PollCreationService`/`PollCleanupService`/`PollExpirationService`/`ParticipantService` calling the *same* write methods were correctly excluded (the healthy/unhealthy contrast on an identical query the field feedback called "the strongest evidence" of real signal-to-noise, not a result tuned to this one codebase). `--fail-on-violation=true --max-violations=0` correctly fails the build. Validated with a fresh, zero-context subagent per the established pattern: independently confirmed every flagged call site against source, found the same healthy-service counter-examples unprompted, and confirmed read-side pass-through (`getPoll` called from the same route files) correctly never appears in the violation list. Zero false positives found.

**Known, documented limit** (explicitly not over-promised in the guidance text): this only catches orchestration that touches a port. Purely in-memory sequencing with no write anywhere in it slips through structurally — every example in the original field report happened to involve a write, so it caught the real ones there, but this is a genuine ceiling on what a port-call-based check can see.

**Tests**: `PortMethodClassifierTest` (new, 5 cases). `TestCouplingBuilderTest` — 6 new (adapter calling a port write is flagged; adapter calling a port read is not; a service-tier caller of the same write is not flagged; the healthy/unhealthy contrast resolves correctly in one run; a port's own implementor is never flagged even if misconfigured into the candidate set; write/read overrides apply to the adapter subject). `TaskRegistryTest`/`HelpTextTest` updated for the three new params.
