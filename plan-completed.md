...
**Follow-up from ra-backend's `--scope=all` run**: the default scope produced dramatically noisier output than `--scope=prod` (55 findings vs. 12) — 6 ACT NOW + 21 LATENT that were almost entirely test-introduced (`SystemTestContext`/e2e wiring pulling every feature package through one shared hub, which `RingDetector`'s own composition-root auto-detection didn't fully catch in that all-scope run). Fixed two ways:
1. **`cnavConverge` now defaults to `--scope=prod`** (`paramDefaultOverrides = mapOf("scope" to "prod")` on the `CONVERGE` `TaskDef`, and `ConvergeConfig.parse` defaults the actual runtime value the same way — `paramDefaultOverrides` alone is display-only for help text, confirmed by tracing `BalanceConfig`'s existing `top`-default-override precedent, which does the same custom-default-in-`parse()` pattern). Matches the existing agent-facing "canonical flags: `--format=llm` / `--scope=prod`" guidance in `AgentHelpText.kt` — cnavConverge is squarely an "architectural signal" tool, the exact case that guidance targets.
2. **New `--exclude-packages=<regex>` param**, giving a manual backstop for hub packages the composition-root heuristic doesn't catch (or that aren't composition roots at all, like test-only wiring). Deliberately a *new*, distinct param name rather than reusing the existing shared `EXCLUDE` param — investigation found `EXCLUDE` already has inconsistent match semantics across its two existing consumers (`cnavDead` uses full-string `Regex.matches()`; `cnavTestCoupling` uses substring `containsMatchIn()`), and since `cnav-config.json`'s `defaults` section is flat and keyed purely by param name with no per-task allowlist, a single `defaults.exclude` entry would have silently cross-applied to all three tasks at once with three different interpretations. `--exclude-packages` filters the dependency graph *before* cycle/ring detection runs (not just the final edge list) — a hub package's edges need to be gone before SCC computation for the fix to actually work, not just hidden from display afterward. Applies uniformly in both modes: intersect mode drops any `PackageDependency`/resolved coupling pair touching an excluded package; risk mode drops any matching class from the ranking.

New tests: `TestClassWriter`-based orchestrator tests for both the scope-default and the exclude-drops-hub-edges behavior, plus `ConvergeConfigTest` cases for the new default and override. Full suite (2,725 tests) and Maven compile both green.
