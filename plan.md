...
### `cnavRings` field-tested against Spring PetClinic (Java/Maven, Spring Data JPA) — composition-root exclusion, project-namespace collision, Serializable, and Spring Data proxy ports
...
**Noted but deliberately not fixed**: `CrashController`/`WelcomeController` (trivial controllers with no framework-typed parameters/return values — just `@GetMapping` returning a view-name `String`) stay in the innermost ring, since `SignatureTypeScanner` deliberately never visits annotations, only real declared supertypes/field/parameter/return types. A narrow, known scope boundary, not a regression — annotation-only framework usage (no framework-typed values anywhere in the signature) isn't detected.
