...
**ACTIVE (2 of 3 spread-logic bullets already resolved, shared lookup done)** | **Value: medium** | **Effort: high** | Source: internal
...
- Formatter classes: some contain query logic belonging in builders — **still open.** Broader than a single fix; needs an audit across the many TEXT/JSON/LLM formatters (candidate pattern: `.filter`/`.sortedBy` combined with actual domain decisions, not just presentation ordering) to find which ones mix query logic into the formatting layer. Not attempted this pass — the [[cnavAnnotations]]-style Builder/Formatter split is the target shape where this has already been done correctly.
