...
object ConfigHelpText {
    fun generate(tool: BuildTool = BuildTool.GRADLE): String = buildString {
        ...
        appendLine("  ${p("method", "<regex>")}          Method regex (${t("find-callers")}, ${t("find-callees")}, ${t("find-usages")})")
        ...
        appendLine()
        appendLine("--- Type Ranking ---")
        appendLine()
        appendLine("  ${p("top", "<N>")}                 Max results (${t("rank")}, default: 50)")
        appendLine("  ${p("projectonly", "true")}        Filter to project classes only (${t("rank")}, default: true)")
        appendLine()
