...
abstract class ReportTask : DefaultTask() {
    ...
    fun generateReport() {
        ...
        val cliProps = project.buildPropertyMap(TaskRegistry.REPORT)
        val props = extension.resolveProperties(cliProps)

        val format = ParamDef.parseFormat(props)
        val scope = Scope.parse(props["scope"])
        val packageFilter = props["package-filter"]
        val top = props["top"]?.toIntOrNull() ?: 20
        val after = TaskRegistry.AFTER.parseFrom(props)
        val followRenames = props["no-follow"] != "true"

        val taggedDirs = project.taggedClassDirectories()
        val filteredDirs = taggedDirs.filter { scope.matchesSourceSet(it.second) }
        ...
        val filterPkg = packageFilter?.let { no.f12.codenavigator.navigation.types.PackageName(it) }
        ...
        val commits = GitLogRunner.run(project.projectDir, after, followRenames = followRenames)
        val hotspots = HotspotBuilder.build(commits, minRevs = 1, top = top)
        ...
        if (deadCode.isNotEmpty()) {
            sections += "## Dead Code (top $top)\n\n${DeadCodeFormatter.format(deadCode.take(top), scope)}"
        } else {
