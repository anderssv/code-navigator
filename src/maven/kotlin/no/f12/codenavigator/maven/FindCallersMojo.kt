...
class FindCallersMojo : AbstractMojo() {
    ...
    override fun execute() {
        val methodPattern = method
            ?: throw MojoFailureException("Missing required property 'method'. Usage: mvn cnav:find-callers -Dmethod=<regex> -Dmaxdepth=3")
        val depth = maxdepth
            ?: throw MojoFailureException("Missing required property 'maxdepth'. Usage: mvn cnav:find-callers -Dmethod=<regex> -Dmaxdepth=3")
        ...
        val trees = CallTreeBuilder.build(graph, methods, depth, CallDirection.CALLERS, filter)
